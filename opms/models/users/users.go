...
func LoginUser(username, password string) (err error, user Users) {
	...
	fmt.Println(err)
	if err == nil {
		o.Raw("UPDATE pms_users_profile SET lasted = ?,lognum+? WHERE userid = ?", time.Now().Unix(), 1, users.Id).Exec()
	}
	return err, users
}

//得到用户信息
func GetUsers(id int64) (Users, error) {
	var user Users
	var err error
	o := orm.NewOrm()
...
func GetRealname(id int64) string {
	...
	err = utils.GetCache("GetRalname.id"+fmt.Sprintf("%d",id), &realname)
	if err != nil {
		    ...
		    utils.SetCache("GetRealname.id."+fmt.Printf("%d", id), realname, cache_expire)
	}
	return realname
}
...
func GetUserEmail(id int64) string {
	...
	if err != nil {
		   ...
		   var user UserProfile
		   o := orm.NewOrm()
		   o.QueryTable(models.TableName("user_profile")).Filter("userid", id).One(&user, "email")
		   email = user.Email
		   utils.SetCache("GetUserEmail.id."+fmt.Printf("%d", id), email, cache_expire)
